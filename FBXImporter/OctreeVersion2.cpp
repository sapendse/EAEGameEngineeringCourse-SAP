#include "OctreeVersion2.h"
#include <fstream>

static int count = 0;

void OctreeBuilder2::OctreeCreator2::ReadMeshData()
{
	...
	scenefile.close();
	OctreeBuilder2::OctreeCreator2::getInstance()->m_entityCount = entityCount;
	m_meshes = OPT_NEW s_mesh2[entityCount];
	...
	for( unsigned int itr = 0; itr < entityCount; itr++ )
	{
		...
		s_vertex2* vertexData = OPT_NEW s_vertex2[primitiveCount];
		for( unsigned int c=0; c< primitiveCount; c++ )
		{
			s_vertex2 temp;
			meshfile.read( reinterpret_cast<char*>(&temp), sizeof( s_vertex2 ) );
			vertexData[c] = temp;
		}
		OctreeBuilder2::OctreeCreator2::getInstance()->m_meshes[itr].m_primitiveCount = primitiveCount;
		OctreeBuilder2::OctreeCreator2::getInstance()->m_meshes[itr].m_vertices = vertexData;
		unsigned int* indexData = OPT_NEW unsigned int[primitiveCount];
		...
		OctreeBuilder2::OctreeCreator2::getInstance()->m_meshes[itr].m_indexCount = indexCount;
		OctreeBuilder2::OctreeCreator2::getInstance()->m_meshes[itr].m_indices = indexData;

	}
}

void OctreeBuilder2::OctreeCreator2::CreateRootNode()
{
	...
	for( unsigned int i = 0; i < m_entityCount; i++ )
	{
		for( unsigned int j = 0; j < m_meshes[i].m_primitiveCount; j+=3 )
		{
			...
			Triangle2 t = Triangle2( first, second, third );
			m_octree.m_rootNode.m_listOrderedPoints.push_back( t );
		}
	}
}

bool OctreeBuilder2::OctreeCreator2::ShouldPushDown( OctreeNode2& i_node, Optimus::Vector3& i_first, Optimus::Vector3& i_second, Optimus::Vector3& i_third )
{
...
void OctreeBuilder2::OctreeCreator2::FillData( OctreeNode2& i_node, std::vector<Triangle2>& i_listOrderedPoints )
{
	bool erased = false;
	...
	if( ( i_node.m_currentDepth == this->m_octree.m_granLevel ) || ( i_node.m_listOrderedPoints.size() == 0 ) )
		i_node.m_isLeafNode = true;
	else
		i_node.m_isLeafNode = false;

	if( i_node.m_currentDepth < this->m_octree.m_granLevel )
	{
		if( !i_node.m_isLeafNode )
		{
			...
			{
				i_node.m_children.push_back( OctreeNode2() );
...
void OctreeBuilder2::OctreeCreator2::Divide( OctreeNode2& i_node )
{
	for( unsigned int i = 0; i < 8; i++ )
	{
		OctreeNode2 childNode;
		...
		i_node.m_children.push_back( childNode );
		i_node.m_children[i].m_currentDepth = i_node.m_currentDepth + 1;
	}
}

void OctreeBuilder2::OctreeCreator2::SortByDFS( OctreeNode2& i_node, std::vector<OctreeNode2>& i_sortedNodes )
{
	//if( i_node != 0 )
	{
...
void OctreeBuilder2::OctreeCreator2::UnSort( OctreeNode2& i_node, std::vector<OctreeNode2>& i_sortedNodes, int i_gran )
{
	...
	{	
		for( unsigned int i = 0; i < 8; i++ )
		{
			...
			OctreeNode2 newNode;
			i_node.m_children.push_back( newNode );
...
void OctreeBuilder2::OctreeCreator2::WriteData()
{
	std::vector<OctreeNode2> sortedNodes;

	SortByDFS( m_octree.m_rootNode, sortedNodes );

	std::ofstream outFile( "../Assets/Meshes/OctreeMeshTXTTEST.txt", std::ios::out );
	outFile << "Octree Begin\n";
	outFile << "Granularity Level:" << m_octree.m_granLevel <<"\n";
	...
	int gran = m_octree.m_granLevel;

	std::ofstream outBinFile( "../Assets/Meshes/OctreeMeshTEST.bin", std::ios::out | std::ios::binary );
...
void OctreeBuilder2::OctreeCreator2::ReadOctree()
{
	std::vector<OctreeNode2> i_sortedList;
	
	std::ifstream inBinFile( "../Assets/Meshes/OctreeMeshTEST.bin", std::ios::in | std::ios::binary );
	...
	for( unsigned int i = 0; i< octreeSize; i++ )
	{
		...
		if( trianSize != 0 )
		{
			for( int j = 0; j < trianSize; j++ )
			{
				...
				node.m_listOrderedPoints.push_back( Triangle2( Optimus::Vector3( p1_x, p1_y, p1_z ), Optimus::Vector3( p2_x, p2_y, p2_z ), Optimus::Vector3( p3_x, p3_y, p3_z ) ) );
			}
		}
	...
	m_octree = Octree2( OctreeNode2(), gran );

	UnSort( m_octree.m_rootNode, i_sortedList, gran );
}

void OctreeBuilder2::OctreeCreator2::CreateOctree()
{
	ReadMeshData();

	m_octree = Octree2( OctreeNode2(), 3 );
